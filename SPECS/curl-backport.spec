%define _libdir %{_prefix}/lib64/curl-backport
...
Patch200: curl-7.50.3-libidn-valgrind-rhel7.patch

Provides: webclient
URL: http://curl.haxx.se/
BuildRoot: %{_tmppath}/curl-%{version}-%{release}-root-%(id -nu)
BuildRequires: gcc
BuildRequires: groff
...
Requires: libcurl-backport%{?_isa} = %{version}-%{release}
...
%package -n libcurl-backport-devel
...
Provides: curl-backport-devel = %{version}-%{release}
Provides: curl-backport-devel%{?_isa} = %{version}-%{release}
Obsoletes: curl-backport-devel < %{version}-%{release}

%description -n libcurl-backport-devel
...
# rename for parallel installation
mv $RPM_BUILD_ROOT/usr/bin/curl{,-backport}
mv $RPM_BUILD_ROOT/usr/bin/curl{,-backport}-config
...
install -m 644 docs/libcurl/libcurl.m4 $RPM_BUILD_ROOT%{_datadir}/aclocal/libcurl-backport.m4
rm -f $RPM_BUILD_ROOT%{_datadir}/aclocal/libcurl.m4

mv $RPM_BUILD_ROOT%{_mandir}/man1/curl{,-backport}.1
mv $RPM_BUILD_ROOT%{_mandir}/man1/curl{,-backport}-config.1
...
# rename include
mv $RPM_BUILD_ROOT/usr/include/curl{,-backport}

%clean
rm -rf $RPM_BUILD_ROOT

#% post -n libcurl-backport -p /sbin/ldconfig

#% postun -n libcurl-backport -p /sbin/ldconfig
...
%{_bindir}/curl-backport
%{_mandir}/man1/curl-backport.1*
#%{_datadir}/zsh/site-functions

%files -n libcurl-backport
%{!?_licensedir:%global license %%doc}
%license COPYING
%{_libdir}/libcurl.so.*

%files -n libcurl-backport-devel
%doc docs/examples/*.c docs/examples/Makefile.example docs/INTERNALS.md
%doc docs/CONTRIBUTE.md docs/libcurl/ABI
%{_bindir}/curl-backport-config*
%{_includedir}/curl-backport
%{_libdir}/*.so
%{_libdir}/pkgconfig/*.pc
%{_mandir}/man1/curl-backport-config.1*
#%{_mandir}/man3/*
%{_datadir}/aclocal/libcurl-backport.m4

%changelog
* Tue Sep 20 2016 Markus Frosch <markus.frosch@netways.de> 7.50.3-1
- Backport for RHEL7, as curl-backport
- Add patch curl-7.50.3-libidn-valgrind-rhel7.patch to avoid valgrind false-positives
- Disable metalink, psl and nghttp2 support for RHEL7
